...
var Channel_text,Country_text,Channel_warn;		
...
var country2 = new Array ("Auto",36,40,44,48,149,153,157,161);//Australia, Canada  美国印度马来西亚 墨西哥  中东(Saudi Arabia)    新加坡    南美洲  
var country4 = new Array ("Auto",149,153,157,161);//中国 中东(Iran/Labanon/Qatar)
var country5 = new Array ("Auto",36,40,44,48,52,56,60,100,104,108,112,116,132,136);//Europe 
var country8 = new Array ("Auto",36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136);//日本
var country9 = new Array ("Auto",36,40,44,48,149,153,157,161);//韩国
var country19=new Array ("Auto",56,60,64,149,153,157,161);//台湾
...
var ezairmode_index = 0,ezairmode_select = 0,select_txt;
...
function set_appsk() {
    ...
    if(ezmusic_flag==6)//pro box
    {
            var ignoressid_value=Number($("#ignoressid_val").val());
	     if ((appskValue != appskValue_save)||(ignore_ssid_flag!=ignoressid_value))
	    {
		...
		else 
		{
	        ...
	        if (!errorFlag) 
		 {
	            ...
	            change_psw_ignoressid();
	            
	        }
	    }
    }
    else if (appskValue != appskValue_save)
    {
    	if (!regx.test(appskValue))
	{
            $('#psk_warn_txt').text(PSW_ALLOW_string);
    ...
    {
     	$('#psk_warn_txt').text("Please enter a new password to save again");
    }
}
//密码设置按键响应
...
//读取ssid开关状态
function get_ignore_ssid()
{
    var value;
    var cgiurl = "cgi-bin/wifi_info_GET.cgi?timeStamp=" + new Date().getTime();
    $.get(cgiurl, {type:'get_ignoressid'}, function(ret) {	
        value=Number(ret);
	  ignore_ssid_flag=value;
...
function Get_index_language() {
    ...
    $.get(cgiurl, {type: 'get_index_text'},function(index_text_string) {
        ...
        ok_string = $('#upgrade_OK').val();
        cancle_string = $('#upgrade_CANCEL').val();
        ...
        $('#upgrade_OK').text($('#upgrade_txt').text());
...
function Get_language_txt() {
    ...
    $.get(cgiurl, {type: 'get_lan_text'},function(lan_text) {
        if (lan_text.length) {
            ...
            text_string = text_string_global;
            device_name_warn1 = Get_text_from_textstring(text_string);//The device name can not be more than 15 characters
            text_string = text_string_global;
...
function Get_AP_mac_address() {
    $.get("cgi-bin/wifi_info_GET.cgi", {type: 'get_mac_ip'},function(mac_addr) {
        if (mac_addr.length) {
            start = 0;
            text_string_length = 0;
            text_string = mac_addr;
            text_string_length = mac_addr.length;
...
function Get_Lan_mac_ip() {
    $.get("cgi-bin/wifi_info_GET.cgi", {type: 'getLan_mac_ip'},function(mac_lan_ip) {
        if (mac_lan_ip.length) {
            $('#connect_mac_address').text(mac_lan_ip);
        }
    },"text");
}
...
function Channel_listener() {
    ...
    $("#set_5Gchannel_ok").click(function() {
        if($('#5G_Channel_val').val()=="Auto")
            ...
            channel_5G_index = Number($('#5G_Channel_val').val());

        get_channel_5G_country_index = Number($('#5G_country_val').val());
...
function set_Lanmanual_info() {
    if (check_ip() == 0) {
        ...
        var str = $('#ipaddr').val() + "\n" + $('#netmask').val() + "\n" + $('#gateway').val() + "\n" + $('#dns1').val() + "\n" + $('#dns2').val() + "\n" + "writeLanManualInfo\n";
        $.get("cgi-bin/wifi_info_GET.cgi", {type: str},function(flag) {
            if (flag == "-1" || flag == "0"){
...
function set_LanAutomatic_off() {
    $.get("cgi-bin/wifi_info_GET.cgi", {type: 'setLanAuto_off'},function() {
    },"text");
}

function set_LanAutomatic() {
    $.get("cgi-bin/wifi_info_GET.cgi", {type: 'setLanAutomatic'},function() {
    },"text");
}
...
function get_LanAutomatic() {
    $.get("cgi-bin/wifi_info_GET.cgi", {type: 'getLanAutomatic'},function(LanAutomatic) {
...
function get_LanSetting_auto_ip() {
    $.get("cgi-bin/wifi_info_GET.cgi", {type: 'getLanSettingautoip'},function(LanSettingVal) {
        var ret = LanSettingVal;
    ...
    $.get("cgi-bin/wifi_info_GET.cgi", {type: 'EZMUSIC_ENABLE'},function(ezmusic_enable) {
        ...
        switch (ezmusic_flag) {
            ...
            case 7:
                ...
                $("#ignoressid_control").remove();
                $("#for_ezmu_esca").css('display', 'block');
                $("#modelout,#Set_EQ_mode,#wifilist,#lanlist,#pass_sel,#device_sel,#ezchannelmode,#channel_set,#foot_cus").remove();
                Get_resolution();
                //Get_connected_ssid();
                break;
            default:
        ...
        if (ezmusic_flag == 3||ezmusic_flag ==5||ezmusic_flag == 6){
            Check_lan_in();
        } 
...
function delay_get_data_function() {
    ...
    delay_get_data_id = self.setInterval(click_listener, 700);
